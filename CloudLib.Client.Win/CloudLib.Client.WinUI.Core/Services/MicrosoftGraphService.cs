...
namespace CloudLib.Client.WinUI.Core.Services
{
    public class MicrosoftGraphService
    {
        ...
        {
            User? user = null;
            var httpContent = await GetDataAsync($"{GraphApiEndpoint}{_apiServiceMe}", accessToken);
            if (httpContent != null)
            {
                var userData = await httpContent.ReadAsStreamAsync();
        ...
        {
            var httpContent = await GetDataAsync($"{GraphApiEndpoint}{_apiServiceMePhoto}", accessToken);
